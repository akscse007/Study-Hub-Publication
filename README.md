...
- **Admin panel** at `/admin` — dashboard, books CRUD, inventory, announcements, WhatsApp leads, activity logs, Landing Page controls (readers counter + hero image uploads), settings, admin user management with role hierarchy (`subadmin` < `superadmin` < `developer`).
...
- **Live updates & new-book alerts** — the site keeps a Server-Sent Events connection open: the catalogue refreshes automatically when books change, and visitors who granted browser-notification permission get an alert when a new book is published.
...
| `CLIENT_URL` | yes in production (server refuses to start without it); dev defaults to `http://localhost:5173` | Public site origin, used for CORS |
...
Never commit `server/.env`. It is listed in `.gitignore`; if a copy was ever committed or shared, rotate `JWT_SECRET`.
...
- Public form endpoints (WhatsApp leads) are rate-limited and field-whitelisted.
...
- Note: startup fails fast if MongoDB is unreachable — check logs on first deploy.
